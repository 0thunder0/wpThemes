...
jQuery().ready(function() {
    ...
    //color click
    jQuery('.td-set-color').click(function(){

        var td_custom_css_parser = new window.td_custom_css_parser(td_style_buffer);

        var td_cur_color = jQuery(this).data('td-color');

        td_custom_css_parser.load_setting_raw('theme_color', td_cur_color);
        td_custom_css_parser.load_setting_raw('link_color', td_cur_color);
        td_custom_css_parser.load_setting_raw('link_hover_color', td_cur_color);

        //slider text
        td_custom_css_parser.load_setting_raw('slider_text', td_convert_hex(td_cur_color, 70));



        td_custom_css_parser.load_setting_raw('select_color', lighterColor(td_cur_color, .3));



        //td_custom_css_parser.load_setting_raw('theme_color', td_convert_hex(jQuery(this).data('td-color'), 70));
    ...
    jQuery('.td-customizer-change-layout').click(function(){

        if (jQuery(this).data('cur-layout') == 'boxed') {
            //change to full
            jQuery('.td-customizer-change-layout span').text('boxed');
            jQuery('#td-theme-settings').removeClass("td-theme-settings-show-bg");

            jQuery(this).data('cur-layout', 'full');
            jQuery(this).removeClass('td-change-to-full');
            jQuery(this).addClass('td-change-to-boxed');


            jQuery('body').removeClass('td-boxed-layout');
            jQuery('body').addClass('td-full-layout');





            //for big slide + bg template

                jQuery.backstretch("destroy", false);
                var cur_backstratch_image = jQuery('.backstretch img').attr('src');

                jQuery('.backstretch').remove();
                if (cur_backstratch_image != '' && typeof cur_backstratch_image !== 'undefined') {

                    jQuery('.td-big-slide-background').backstretch(cur_backstratch_image);
                }
            ...
            jQuery('.td-theme-settings-no-transition').removeClass('td-theme-settings-no-transition');

            jQuery('.td-menu-wrap').css('transition', 'none');

            jQuery('.td-customizer-change-layout span').text('full');
            jQuery('#td-theme-settings').addClass("td-theme-settings-show-bg");





            jQuery(this).data('cur-layout', 'boxed');
            jQuery(this).removeClass('td-change-to-boxed');
            jQuery(this).addClass('td-change-to-full');

            jQuery('body').removeClass('td-full-layout');
            jQuery('body').addClass('td-boxed-layout');


            //on the slide + background template
            if (jQuery(".backstretch")[0]){
                jQuery.backstretch("destroy", false);
                var cur_backstratch_image = jQuery('.backstretch img').attr('src');
                jQuery('.backstretch').remove();
                jQuery('.td-big-slide-background .container').backstretch(cur_backstratch_image);
            }


            jQuery.backstretch(td_bg_boxed_imgs, {fade:1200});
    ...
    jQuery('.td-bg-changer').click(function(){
        jQuery.backstretch('show', jQuery(this).data('bg'));
    });
