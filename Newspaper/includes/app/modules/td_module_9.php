...
class td_module_9 extends td_module {
    ...
    function render() {
        $buffy = '';
        $buffy .= '<div class="td_mod_wrap td_mod9 ' . $this->get_no_thumb_class() . '" ' . $this->get_item_scope() . '>';
        $buffy .= $this->get_image('art-big-1col');

        $buffy .= '<div class="item-details">';
        $buffy .= $this->get_title(td_util::get_option('tds_mod9_title_excerpt'));
        ...
        $buffy .= '<p>' .$this->get_excerpt(td_util::get_option('tds_mod9_content_excerpt')) . '</p>';
